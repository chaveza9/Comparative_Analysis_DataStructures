...
void quadtree::delete_node(Node *Pt){
    ...
    if(Q==NULL){
        // Point does not exist within this quadtree
        printf("point does not exist and cannot be deleted");
...
void quadtree::show(cv::Mat *window){
    //create Boundary Rectangles
    cv::rectangle( *window, cv::Point( this->top_left.x, this->top_left.y),
         cv::Point( this->bot_right.x, this->bot_right.y), cv::Scalar( 0, 0, 0 ));
    // Traverse Through graph
    for (int i = 0; i < 4; i++)
    {
        if(this->child[i]!= NULL){
            this->child[i]->show(window);
        }
        else
        {
            return;
...
void quadtree::drawInRegion(cv::Mat *img, Point topLeft, Point botRight){
    //define empty storing stacks
    Stack* X = createStack(STACK_CAPACITY);
    Node N;
    // Extract values in region
    ...
    while(X->size!=0)
    {
        N = Pop(X);
        printf("x=%d \t y=%d \t val=%d \n", N.coordinates.x, N.coordinates.y, N.data);
        cv::circle(*img,  cv::Point(N.coordinates.x,N.coordinates.y), 3, cv::Scalar(18,173,42));
        cv::rectangle( *img, cv::Point( topLeft.x, topLeft.y),
         cv::Point( botRight.x, botRight.y), cv::Scalar( 255, 62, 51 ));       
    }
